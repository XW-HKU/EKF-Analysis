...
% periodogram(acc_gryo_in(:,2),rectwin(length(acc_gryo_in(:,2))),length(acc_gryo_in(:,2)),IMU_Fr);
[pxx,f] = pwelch(acc_gryo_in(:,2),[],[],[],IMU_Fr);
loglog(f,pxx);
title('IMU measurements');
subplot(1,3,2);
% periodogram(acc_butfilt(:,1),rectwin(length(acc_butfilt(:,1))),length(acc_butfilt(:,1)),IMU_Fr);
